...
public class InMemoryMenuAdminService : IMenuAdminService
{
    private readonly InMemoryDataStore _db;
    
    public InMemoryMenuAdminService(InMemoryDataStore db) 
    { 
        _db = db; 
    }
    ...
    {
        var m = _db.Menu.FirstOrDefault(x => x.Id == id);
    ...
    {
        var permissions = _db.Permissions.Where(p => requiredPermissionCodes.Contains(p.Code)).ToList();
    ...
    {
        var item = _db.Menu.FirstOrDefault(m => m.Id == id);
        if (item == null) return false;
        
        item.Title = title;
        item.TitleEn = titleEn ?? item.TitleEn ?? title;
        item.TitleAr = titleAr ?? item.TitleAr ?? title;
        item.Url = url; 
        item.ParentId = parentId; 
        item.Order = order; 
        item.IconId = iconId;
        
        var newPerms = _db.Permissions.Where(p => requiredPermissionCodes.Contains(p.Code)).ToList();
